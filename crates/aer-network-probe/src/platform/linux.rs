...
fn snapshot_family(
    ...
    if raw_fd < 0 {
        return Err(AdapterError::PermissionLimited);
    }
    ...
    let sequence = match family {
        AddressFamily::Ipv4 => 0x1a4_u32,
        AddressFamily::Ipv6 => 0x1a6_u32,
    };
...
fn parse_netlink(
    ...
    while offset < buffer.len() {
        if buffer.len() - offset < NL_HEADER {
        ...
        if message_type != SOCK_DIAG_BY_FAMILY || length < NL_HEADER + DIAG_MESSAGE {
            return Err(AdapterError::MalformedResponse);
        }
        let payload = &buffer[offset + NL_HEADER..offset + length];
        if let Some(row) = parse_diag(payload, family, request, owners)? {
            output.push(row);
        }
        offset = align4(
            offset
...
fn parse_diag(
    ...
    let Some((fixture, allowed_uid)) = owners.get(&inode).copied() else {
        return Ok(None);
    };
    let association = if uid == allowed_uid {
    ...
    Ok(Some(RawRow {
        ...
        owner: PlatformOwner::Linux { uid, inode },
        association,
    }))
}

fn allowed_socket_inodes(
    request: &SnapshotRequest,
) -> Result<BTreeMap<u64, (FixtureLabel, u32)>, AdapterError> {
    let mut output = BTreeMap::new();
    for identity in &request.processes {
        let AllowListedProcess::Linux { fixture, pid, uid } = identity else {
            continue;
        };
        let process = PathBuf::from(format!("/proc/{pid}"));
        let metadata = fs::metadata(&process).map_err(|error| {
            if error.kind() == std::io::ErrorKind::PermissionDenied {
                AdapterError::PermissionLimited
            } else {
        ...
        let descriptors = fs::read_dir(process.join("fd")).map_err(|error| {
            if error.kind() == std::io::ErrorKind::PermissionDenied {
                AdapterError::PermissionLimited
            } else {
                AdapterError::PlatformFailure
        ...
        for descriptor in descriptors {
            ...
            let target =
                fs::read_link(descriptor.path()).map_err(|_| AdapterError::BufferChanged)?;
            if let Some(inode) = parse_socket_link(&target) {
                output.insert(inode, (*fixture, *uid));
            }
        }
    }
    Ok(output)
...
mod tests {
    ...
    fn socket_link_parser_accepts_only_numeric_descriptor_targets() {
        assert_eq!(
            parse_socket_link(std::path::Path::new("socket:[42]")),
            Some(42)
        );
