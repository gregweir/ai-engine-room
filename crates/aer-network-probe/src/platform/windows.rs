...
fn collect_ipv4(request: &SnapshotRequest, output: &mut Vec<RawRow>) -> Result<(), AdapterError> {
    ...
    parse_table::<MIB_TCPROW_OWNER_PID, _>(
        &buffer,
        |row| {
            ...
            Ok(Some(RawRow {
                ...
                remote: NumericEndpoint {
                    address: IpAddr::V4(Ipv4Addr::from(row.dwRemoteAddr.to_ne_bytes())),
                    port: port(row.dwRemotePort),
                },
                owner: PlatformOwner::Windows {
                    pid: row.dwOwningPid,
                    image_path,
                },
                association,
...
fn collect_ipv6(request: &SnapshotRequest, output: &mut Vec<RawRow>) -> Result<(), AdapterError> {
    ...
    parse_table::<MIB_TCP6ROW_OWNER_PID, _>(
        &buffer,
        |row| {
            let Some((fixture, expected_path)) = windows_identity(request, row.dwOwningPid) else {
                return Ok(None);
            ...
            Ok(Some(RawRow {
                ...
                remote: NumericEndpoint {
                    address: IpAddr::V6(Ipv6Addr::from(row.ucRemoteAddr)),
                    port: port(row.dwRemotePort),
                },
                owner: PlatformOwner::Windows {
                    pid: row.dwOwningPid,
...
fn tcp_table(family: u32) -> Result<Vec<u8>, AdapterError> {
    ...
    if status != NO_ERROR || length as usize > buffer.len() {
        return Err(AdapterError::PlatformFailure);
    }
...
fn parse_table<T: Copy, F>(
    buffer: &[u8],
    mut project: F,
    output: &mut Vec<RawRow>,
) -> Result<(), AdapterError>
where
    F: FnMut(T) -> Result<Option<RawRow>, AdapterError>,
{
    ...
    let bytes = count
        ...
        .and_then(|n| n.checked_add(size_of::<u32>()))
        .ok_or(AdapterError::ResourceLimit)?;
    if count > MAX_RAW_ROWS || bytes > buffer.len() {
        return Err(AdapterError::MalformedResponse);
    }
...
fn windows_identity(request: &SnapshotRequest, pid: u32) -> Option<(FixtureLabel, &Path)> {
    request
        ...
        .find_map(|identity| match identity {
            AllowListedProcess::Windows {
                fixture,
                pid: allowed,
                expected_image_path,
            } if *allowed == pid => Some((*fixture, expected_image_path.as_path())),
            _ => None,
        })
}
...
fn validate_image(pid: u32, expected: &Path) -> (PathBuf, ProcessAssociation) {
    match image_path(pid) {
        ...
        Err(AdapterError::PermissionLimited) => {
            (PathBuf::new(), ProcessAssociation::PermissionLimited)
        }
...
fn image_path(pid: u32) -> Result<PathBuf, AdapterError> {
    ...
    if ok == 0 || length as usize > buffer.len() {
        return Err(AdapterError::PlatformFailure);
    }
    buffer.truncate(length as usize);
    Ok(PathBuf::from(
        String::from_utf16(&buffer).map_err(|_| AdapterError::MalformedResponse)?,
    ))
}
...
mod tests {
    ...
    fn table_parser_rejects_truncated_artificial_rows_without_native_io() {
        ...
        let result = parse_table::<MIB_TCPROW_OWNER_PID, _>(&buffer, |_row| Ok(None), &mut output);
